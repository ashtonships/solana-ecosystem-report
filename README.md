...
keyless endpoints (CoinGecko, DeFiLlama)**, records **releases, proposal
activity and status incidents from official keyless feeds**, detects anomalies
against accumulated history, and renders a dark HTML dashboard with **inline
SVG trend charts drawn from the committed snapshot history**, a human-readable
...
**what changed since the previous snapshot, trends across the whole snapshot
history, and the ecosystem's own release and status feeds** — from one command,
...
python3 collect.py --no-news              # skip the release/proposal/status feeds
...
news.py      ─┤  keyless release, proposal and status feeds
              ▼
collect.py  ──► snapshots/snapshot-<UTC>.json   (append-only history)
                snapshots/latest.json           (copy, not a symlink)
                        ...
                        ├──► delta.py           what changed since the last snapshot
                        ├──► charts.py          inline SVG trends across history
                        │            │
...
time, delinquency, SOL price, TVL, median non-vote fee, and estimated 24h REV.
...
**Only real committed snapshots are plotted.** Nothing is smoothed, resampled
or back-filled, and a series with fewer than two real points is not charted at
all — it is listed as "not charted yet, and not drawn as zero".
...
`news.py` records what the ecosystem is publishing, from official first-party
feeds that need no credentials. Each was verified by direct unauthenticated
request before code was written against it:

| Feed | Provides |
| --- | --- |
| `github.com/anza-xyz/agave/releases.atom` | validator client releases |
| `…/solana-improvement-documents/commits/main.atom` | SIMD proposal activity |
| `status.solana.com/history.atom` | official incident history |

One candidate was **dropped**: the SIMD repository's *releases* feed answers
200 and publishes zero entries, because that repository tags no releases. A
permanently empty panel reads as a bug, so its commit feed — which carries the
actual proposal activity — is used instead. That feed shows drafting, not
acceptance, and the dashboard says so.

Fetched items are recorded into the snapshot, so rendering never touches the
network and a committed snapshot re-renders identically offline months later.
Tests run against committed fixtures captured from the real feeds, so an
upstream shape change fails a test rather than quietly emptying a panel.

**Three states are kept distinct**, because collapsing them is how this kind of
section lies:

| State | Renders as |
| --- | --- |
| unreachable or unparseable | *unavailable*, with the reason |
| parsed, zero entries | "read successfully; published no entries" |
| parsed, with entries | the entries, dated and linked |

When every feed fails, the panel says explicitly that this is a statement about
the fetch and not about the ecosystem. Run `python3 collect.py --no-news` to
skip the feeds entirely.

## Output formats

- **`dist/index.html`** — dark-theme dashboard with inline SVG charts, one
  self-contained file with inline CSS. No CDN, no external asset, no script, no
  build step; opens from `file://` with the network down.
...
```bash
python3 -m unittest discover -s tests -v   # or: python3 -m pytest -q
```

252 tests, no network required. They cover the derived metrics (TPS, slot time,
...
- **The dashboard fetches no subresource** — no script, link, img, `@import` or
  `url()`. It draws itself completely from `file://` with the network down.
...
Items that started here and are now built and documented above: **anomaly
detection** (seven detectors over accumulated history), the **keyless economic
sources** (CoinGecko and DeFiLlama), **historical charts** (inline SVG over the
committed snapshot history), the **snapshot-to-snapshot delta**, and
**ecosystem releases and announcements** — the last of which this README
previously called impossible without scraping. It was not: three official
first-party Atom feeds serve it without credentials, and they are verified in
`news.py`. Still open:

- **Dune ecosystem dashboards** — requires an API key, so gated behind the
  no-credential preference rather than assumed.
- **Economic anomaly detectors** — TVL and price moves are currently surfaced
  by the delta section, not by `detect.py`.
