...
(async()=>{
 ...
 for(const width of [701,1024,1440]){
  ...
  const bounds=await ledger.evaluate(e=>{
   const r=e.getBoundingClientRect(),p=e.closest('[data-desktop-history-panel]').getBoundingClientRect();
   return {left:r.left,right:r.right,parentLeft:p.left,parentRight:p.right,documentWidth:document.documentElement.scrollWidth,viewport:innerWidth};
  });
  assert(bounds.left>=bounds.parentLeft-1&&bounds.right<=bounds.parentRight+1,`${width}: ledger escapes panel ${JSON.stringify(bounds)}`);
  assert(bounds.documentWidth<=bounds.viewport,`${width}: document overflows`);
 ...
 checks.push({darkModeCaptures:Boolean(out)});
